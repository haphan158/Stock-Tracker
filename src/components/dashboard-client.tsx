...
import {
  ...
  SORTED_DEFAULT_DASHBOARD_SYMBOLS,
} from '@/src/lib/dashboard-defaults';
...
/** Stable fallback so `data ?? default` does not use a new `[]` every render (infinite useEffect). */
const EMPTY_STOCK_SEARCH: StockData[] = [];

function sameSymbolList(a: string[], b: string[]) {
  return a.length === b.length && a.every((s, i) => s === b[i]);
}

export interface DashboardClientProps {
  initialStocks: StockData[];
  initialMarketSummary: MarketSummaryData;
}

export function DashboardClient({ initialStocks, initialMarketSummary }: DashboardClientProps) {
  ...
  const searchResults = searchData ?? EMPTY_STOCK_SEARCH;
  ...
  useEffect(() => {
    if (!debouncedSearch) {
      setDisplayedStocks((prev) => {
        const next = [...DEFAULT_DASHBOARD_SYMBOLS];
        return sameSymbolList(prev, next) ? prev : next;
      });
      return;
    }
    if (!searchLoading) {
      const next = searchResults.length > 0 ? searchResults.map((stock) => stock.symbol) : [];
      setDisplayedStocks((prev) => (sameSymbolList(prev, next) ? prev : next));
    }
  ...
  const {
    ...
    initialStocks,
    initialSymbolsKey: SORTED_DEFAULT_DASHBOARD_SYMBOLS,
  });
  const { data: marketSummary, isLoading: marketSummaryLoading } =
    useMarketSummary(initialMarketSummary);
